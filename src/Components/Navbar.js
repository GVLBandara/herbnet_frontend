	...
	return (
		<div className={`relative z-10`}>
			<div className={isHome ? styleHome : styleNotHome}>
				<div
					className={`flex gap-3 items-center mr-16 cursor-pointer`}
					onClick={() => {
						navigate('/');
					}}
				>
				...
				{!isHome ? (
					<div
						className={`bg-white h-11 w-full flex justify-between items-center mx-12 rounded-[10px]`}
					>
						<input
				...
				<div className={`flex gap-4 items-center`}>
					...
					{userIsAuthenticated() ? (
						<div className={`flex gap-4 items-center`}>
							...
							/>
							<IoIosArrowDown
								className={`text-3xl text-white cursor-pointer transition-all duration-500 ${
			...
			{resultVisible ? (
				<div>
					<ViewResult close={openResult} plantData={plantData} />
				</div>
			) : (
				<></>
			)}
			{logInVisible ? (
				<div>
					<Login close={openLogin} signup={openSignup} />
				</div>
			) : (
				<></>
			)}
			{singUpVisible ? (
				<div>
					<SignUp close={openSignup} login={openLogin} />
				</div>
			) : (
				<></>
			)}
